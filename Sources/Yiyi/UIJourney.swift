...
@MainActor func runUIJourney(outdir: URL) throws {
    ...
    try check("hyper.not-in-commands", controller.control(accessibilityID: "superkey.popup") == nil && controller.control(accessibilityID: "command.0.hotkey-mode") == nil)
    ...
    try check("shortcut.hyper-hint", try view("command.0.hotkey-hint", as: NSTextField.self).stringValue.contains("Hyper Key"))
    try press("settings.save")
    try check("hyper.off-blocks-save", try view("settings.status", as: NSTextField.self).stringValue.contains("Hyper"))
    try press("command.0.hyper-link")
    try check("hyper.link-opens-general", controller.control(accessibilityID: "section.keyboard") != nil && controller.control(accessibilityID: "section.mouse-/-trackpad") != nil && controller.control(accessibilityID: "superkey.popup") != nil && controller.control(accessibilityID: "pointer.enabled") != nil)
