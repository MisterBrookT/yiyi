...
@MainActor func runUIJourney(outdir: URL) throws {
    ...
    try check("triggers.in-commands", controller.control(accessibilityID: "superkey.popup") != nil && controller.control(accessibilityID: "command.0.hyper-hotkey") != nil && controller.control(accessibilityID: "command.0.pointer") != nil)
    ...
    try check("hyper.recorder-disabled-without-key", try !view("command.0.hyper-hotkey", as: NSButton.self).isEnabled)
    controller.commitRecordedHotkey(accessibilityID: "command.0.hyper-hotkey", value: "super+-")
    try press("settings.save")
    ...
    try check("hyper.keyboard-slot-cleared", try view("command.0.hotkey", as: NSButton.self).title == "Record Shortcut")
