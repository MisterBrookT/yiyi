...
Base URLs must use HTTP(S), without embedded credentials, query strings, or fragments. Use the API root (usually `/v1`), not the endpoint path. The model must match a model exposed by the endpoint. Provider-specific reasoning support varies; the Connection tab exposes reasoning effort. `temperature` and `apiKeyEnv` are config-file fields; an absent temperature is omitted from requests. On Anthropic, reasoning effort maps to an extended-thinking budget (minimal 1k, low 2k, medium 8k, high 16k tokens), `none` disables thinking, and temperature is omitted whenever thinking is on because the API rejects the combination.

Commands support optional `provider`, `model`, and `reasoningEffort` overrides in the config file; Settings shows only a connection override, so it can be cleared. Prompts must contain a supported input token. Substitution is single-pass: token-looking text inside an input is preserved literally, not substituted again.
